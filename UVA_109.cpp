...
int main() {
    ...
    //double totalarea = 0;
    while(cin >> n && n != -1) {
        vector<point> singledom;
        for(int i = 0; i < n; i++) {
            int x, y;
            cin >> x >> y;
            singledom.push_back(point(x, y));
        }
        vector<point> ch = CH(singledom);
        //totalarea += areap(ch);
        kingdoms.push_back(ch);
    }
    int x, y;
    vector<bool> killed;
    killed.assign(25, false);
    double withoutpower = 0;
    while(cin >> x >> y) {
        point tochk = point(x, y);
        for(int i = 0; i < kingdoms.size(); i++) {
            if(inPolygon(tochk, kingdoms[i]) && !killed[i]) {
                double aread = areap(kingdoms[i]);
                killed[i] = true;
                //totalarea -= aread;
                withoutpower += aread;
            }
