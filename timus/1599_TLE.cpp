...
double angle(line &l1, line &l2) { // returns the smaller angle b/w two lines
...
bool inDisk(vec a, vec b, vec p) {
    return dot(a-p, b-p) < eps;
}
bool onSegment(vec a, vec b, vec p) {
    return collinear(a, b, p) and inDisk(a,b,p);
}
...
bool onPolygonSegment(vec pt, const vector<linesegment> &segs) {
    for (int i = 0; i < segs.size(); i++) {
        linesegment l = segs[i];
        if (liesonWithEq(l, pt)) return true;
    }
    return false;
}
bool onPolygonSegment(vec &pt, vector<vec> &pol) {
    for (int i = 0; i < pol.size() - 1; i++) {
        if (onSegment(pol[i], pol[i + 1], pt)) return true;
    }
    return false;
}
pair<int, bool> windingNumber(vec &pt, const vector<vec> &P) { // Works for both convex and concave. It implements
// winding number algorithm
    double sum = 0; // assume the first vertex is equal to the last vertex
    pair<int, bool> ret;
    ret.second = false;
