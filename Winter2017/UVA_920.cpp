...
int main() {
    ...
    while(t--) {
        ...
        double ans = dist(data[n - 1], data[n - 2]);
        point prev = data[n - 2];
        for(int i = data.size() - 4; i >= 0; i -= 2) {
            point at = data[i];
            if(at.y > prev.y) {
                //add ans
                point a = data[i];
                ...
                line ab;
                pointsToLine(a, b, ab);
                //so we have now line ab
                //have to get line cprev;
                line cp;
                pointsToLine(prev, point(prev.x - 1, prev.y), cp);
                point c;
                areIntersect(ab, cp, c);
                ans += dist(c, at);
                prev = at;
            }
        }
