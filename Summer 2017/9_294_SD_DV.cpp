...
int numOfFactors(int N) {//formula is, if primefactorisation is 2^3*5^3*11 then number of factors 
//is (3+1)*(3+1)*(1+1)
cnt=1;
int t;
...
if (N != 1) cnt*=2; // special case if N is actually a prime
return cnt;
}
int main()
{
	...
	int n,u,l;
	ll ans,num;
	cin>>n;//It is actually stupidity and tremendous time consuming to store number of factors of 
	//each and every number between 1 and 10^9
	while(n--)
	{
		cin>>l>>u;
		ans=0;
		for(int j=l;j<=u;j++)
		{
			if(numOfFactors(j)>ans)
			{
				ans=numOfFactors(j);
				num=j;
			}
		}
		printf("Between %d and %d, %u has a maximum of %u divisors.\n",l,u,num,ans);
	}
    return 0;
}
