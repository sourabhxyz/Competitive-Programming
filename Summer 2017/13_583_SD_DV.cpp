...
vector<long long int> primeFactors(ll N) {
vector<long long int> factors; // vi "primes" (generated by sieve) is optional
...
int main()
{
	...
	while(cin>>n&&n)
	{
		...
		vector<long long int> result = primeFactors(n); // with that, we can factor up to 1000^2 = 1000000
		vector<long long int>::iterator it=result.begin();
		if(neg)
		cout<<" x "<<*it;
		else
		cout<<" "<<*it;
